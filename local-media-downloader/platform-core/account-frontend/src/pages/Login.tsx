...
export default function Login() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(t(mode === "signup" ? "login.signupError" : "login.error"));
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="bg-aurora flex min-h-screen items-center justify-center px-4">
      <form onSubmit={handleSubmit} className="glass-panel w-full max-w-sm p-8">
        ...
        <p className="mt-1 text-sm text-slate-400">{t(mode === "signup" ? "login.signupSubheading" : "login.subheading")}</p>
        ...
        >
          {t(mode === "signup" ? "login.switchToLogin" : "login.switchToSignup")}
        </button>
      </form>
    </div>
  );
