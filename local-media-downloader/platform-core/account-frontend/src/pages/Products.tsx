...
export default function Products() {
  ...
  const memberProductIds = new Set(memberships.map((m) => m.product_id));
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {products.filter((p) => memberProductIds.has(p.id)).map((product) => {
          ...
          {products.filter((p) => memberProductIds.has(p.id)).length === 0 && (
      ...
      <div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {products.filter((p) => !memberProductIds.has(p.id) && p.status === "live").map((product) => (
