...
# corruption test).
#
# Usage: scripts/platform/verify-backup-restorable.sh <backup-run-dir>
...
if [[ -f "$RUN_DIR/loady_postgres.dump" ]]; then
  restore_dump_isolated "loady" "$RUN_DIR/loady_postgres.dump"
fi
if [[ -f "$RUN_DIR/platform_core_postgres.dump" ]]; then
  restore_dump_isolated "platform" "$RUN_DIR/platform_core_postgres.dump"
fi

if [[ -f "$RUN_DIR/loady_app.db" ]]; then
  echo "==> Verifying Loady history/settings SQLite file integrity..."
  cp "$RUN_DIR/loady_app.db" "$temp_dir/app.db"
